...
export async function GET() {
  ...
  if (tokensError) {
    ...
    allTokens?.forEach((token, i) => {
      logs.push(
        `  Token ${i + 1}: user=${token.user_id.substring(0, 8)}..., platform=${token.platform}, created=${token.created_at}`,
      );
    });
  }
  ...
  if (linksError) {
    ...
    allLinks?.forEach((link, i) => {
      logs.push(
        `  Link ${i + 1}: parent=${link.parent_id.substring(0, 8)}..., student=${link.student_id.substring(0, 8)}...`,
      );
    });
  }
...
export async function POST(req: NextRequest) {
  ...
  if (!authHeader || !authHeader.startsWith("Bearer ")) {
    return NextResponse.json(
      { error: "Unauthorized - include Bearer token" },
      { status: 401 },
    );
  }
  ...
  if (!vapidPublicKey || !vapidPrivateKey) {
    ...
    return NextResponse.json(
      { logs, error: "VAPID not configured" },
      { status: 500 },
    );
  }
